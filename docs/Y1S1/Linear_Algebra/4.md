...
**Theorem 4.1.** The determinant of an $n \times n$ matrix is a linear function of each row when the remaining rows are held fixed.  That is, for $1 \le r \le n$, we have
$$
\begin{equation*}
\det
\begin{pmatrix}
...
**Theorem 4.2.** If $A \in M_{n \times n}(F)$ has a row consisting entirely of zeros,  then  
$$
\begin{equation*}
\det(A) = 0.
\end{equation*}
...
**Theorem 4.3.** The determinant of a square matrix can be evaluated by cofactor expansion along any row. That is, if $A \in M_{n \times n}(F)$, then for any integer  $i$ ($1 \le i \le n$),
$$
\begin{equation*}
...
**Theorem 4.4.** If $A \in M_{n \times n}(F)$ and $B$ is a matrix obtained from $A$ by interchanging any two rows of $A$, then
$$
\begin{equation*}
\det(B) = -\det(A).
\end{equation*}
...
**Theorem 4.5.** Let $A \in M_{n \times n}(F)$, and let $B$ be a matrix obtained by  adding a multiple of one row of $A$ to another row of $A$.  Then
$$
\begin{equation*}
\det(B) = \det(A).
\end{equation*}
...
**Theorem 4.6.** If $A \in M_{n \times n}(F)$ has rank less than $n$,  then
$$
\begin{equation*}
\det(A) = 0.
\end{equation*}
...
**Theorem 4.7.** For any $A, B \in M_{n \times n}(F)$, we have
$$
\begin{equation*}
\det(AB) = \det(A)\,\det(B).
\end{equation*}
...
**Theorem 4.8.** For any $A \in M_{n \times n}(F)$, we have
$$
\begin{equation*}
\det(A^{t}) = \det(A).
\end{equation*}
...
**Theorem 4.9 (Cramer's Rule).** Let $Ax = b$ be the matrix form of a system of $n$ linear equations in $n$ unknowns, where  $x = (x_1, x_2, \ldots, x_n)^t. $If $\det(A) \neq 0$, then the system has a unique solution, and for each $k$ ($k = 1,2,\ldots,n$),
$$
\begin{equation*}
...
**Theorem 4.10.** For invertible $A$, its inverse $A^{-1}$ may be described as
$$
\begin{equation*}
