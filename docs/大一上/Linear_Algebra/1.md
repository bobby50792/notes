...
A subset $W$ of a vector space $V$ over a field $F$ is called a **subspace** of $V$ with the operations of addition and scalar multiplication defined on $V$ satisfies the following axioms:

1. $0 \in W$.
2. $\forall x, y \in W$, $x + y \in W$.
3. $\forall a \in F, \forall x \in W$, $ax \in W$.

**Theorem 1.** Any intersection of subspaces of a vector space $V$ is a subspace of $V$.

## 1.4 Linear Combinations

### Linear Combination

Let $V$ be a vector space and $S$ be a nonempty subset of $V$. A vector $v \in V$ is called a **linear combination** of $S$. if there exist a finite number of vectors $u_1, u_2, \ldots, u_n \in S$ and scalars $a_1, a_2, \ldots, a_n \in F$ such that $v = a_1u_1 + a_2u_2 + \cdots + a_nu_n$.
...
**Theorem 1.** Let $V$ be a vector space, and let $S_1 \subseteq S_2 \subseteq V$.  If $S_2$ is linearly independent, then $S_1$ is linearly independent.

**Theorem 2.** Let $S$ be a linearly independent subset of a vector space $V$,  and let $v$ be a vector in $V$ that is not in $S$.  Then $S \cup \{v\}$ is linearly dependent if and only if $v \in \text{span}(S)$.
...
**Theorem 1.** Let $V$ be a vector space and $\beta = \{u_1, u_2, \ldots, u_n\}$ be a subset of $V$. Then $\beta$ is a basis for $V$ if and only if each $v \in V$ can be uniquely expressed as a linear combination of vectors of $\beta$,  that is, can be expressed in the form $v = a_1u_1 + a_2u_2 + \cdots + a_nu_n$ for unique scalars $a_1, a_2, \ldots, a_n$.

**Theorem 2 (Replacement Theorem).** Let $V$ be a vector space that is generated by a set $G$ containing exactly $n$ vectors,  and let $L$ be a linearly independent subset of $V$ containing exactly $m$ vectors.  Then $m \le n$ and there exists a subset $H$ of $G$ containing exactly $n - m$ vectors such that $L \cup H$ generates $V$.

**Corollary 1.** Let $V$ be a vector space having a finite basis.  Then every basis for $V$ contains the same number of vectors.



### Standard Basis

1. In $\text{F}^n$, let  $e_1 = (1, 0, 0, \ldots, 0)$,  $e_2 = (0, 1, 0, \ldots, 0)$,  $\ldots$ ,$e_n = (0, 0, \ldots, 0, 1)$. $\{e_1, e_2, \ldots, e_n\}$ is a basis for $\text{F}^n$ and is called the **standard basis** for $\text{F}^n$.  
...
**Theorem 1.** Let $W$ be a subspace of a finite-dimensional vector space $V$.  Then $W$ is finite-dimensional and $\dim(W) \le \dim(V)$.  Moreover, if $\dim(W) = \dim(V)$, then $V = W$.

**Theorem 2.** If $W$ is a subspace of a finite-dimensional vector space $V$,  then any basis for $W$ can be extended to a basis for $V$.
