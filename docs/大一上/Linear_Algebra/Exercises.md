...
1. Consider a system of linear questions
	...
	where $\sum_{i=1}^n a_i \ne 0$. Determine the relationship between $a_1, a_2, \cdots, a_n$ and $b$ such that
...
11. Let $A$ be an $n \times n$ matrix satisfying the polynomial equation $\sum_{i=0}^{m} b_i A^i = 0$, where the coefficients satisfy the condition $|b_m| > \sum_{i=0}^{m-1} |b_i|.$

	Prove that the equation $2X - XA^2 = AX$ has only the trivial solution $X = 0$.
